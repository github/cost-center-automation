...
class CostCenterManager:
    ...
    def assign_cost_center(self, user: Dict) -> str:
        ...
        Rules:
        - If username is in PRUs exception list → PRUs_allowed_costCenter
        - Otherwise → no_PRUs_costCenter
        """
    ...
    def validate_configuration(self) -> List[str]:
        ...
        if not self.cost_center_no_prus:
            issues.append("no_PRUs_costCenter is not defined")
        
        if not self.cost_center_prus_allowed:
            issues.append("PRUs_allowed_costCenter is not defined")
        
        # Check if cost center IDs are the same (would be confusing)
        if self.cost_center_no_prus == self.cost_center_prus_allowed:
            issues.append("no_PRUs_costCenter and PRUs_allowed_costCenter cannot be the same")
